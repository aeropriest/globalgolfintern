...
export async function GET(request: NextRequest) {
  try {
    ...
    // Sort survey results by timestamp (newest first)
    const sortedSurveyResults = [...surveyResults].sort((a, b) => {
      // Handle Firestore Timestamp objects or Date objects
      const dateA = a.timestamp instanceof Date ? a.timestamp : 
                   (a.timestamp && typeof a.timestamp.toDate === 'function') ? 
                   a.timestamp.toDate() : new Date(a.timestamp as any);
    ...
    // Get recent survey results (last 5)
    const recentSurveyResults = sortedSurveyResults.slice(0, 5);

    // Return dashboard statistics
    return NextResponse.json({
      totalApplications: applications.length,
      totalQuizResults: quizResults.length,
      totalSurveyResults: surveyResults.length,
      recentApplications,
      recentSurveyResults,
