...
const ApplicationForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      //     body: JSON.stringify({
      //       to: [email],
      //       subject: 'Your Application to the Global Golf Internship',
      //       html: emailHtml,
      //     })
      //   });
      //   
  ...
  if (false && formState === 'success' && !showInterviewIframe) {
    ...
    const getSocialLinks = () => {
      ...
      const message = `I've just applied for ${position} with the Global Golf Internship! 🌍 Excited about this opportunity to work with leading clubs worldwide. #GlobalInternship #CareerOpportunity`;
      
      return {
        twitter: `https://twitter.com/intent/tweet?text=${encodeURIComponent(message)}&url=${encodeURIComponent(appUrl)}`,
        linkedin: `https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(appUrl)}&title=Applied for Global Golf Internship&summary=${encodeURIComponent(message)}`,
